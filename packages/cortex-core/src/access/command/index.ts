...
export function createCommand(deps: CommandDeps): CommandApi {
  const stub = (method: string): never => {
    throw new NotImplementedError(
      `cortex-core/access.command.${method} is a v0.1 scaffold stub; real implementation lands in v0.x`,
    );
  };
  ...
  const updateMemory = async (
    ...
    deps.logger.debug('cortex.command.updateMemory', {
      id,
  ...
  return {
    upsertEntity: async (entity: Entity) => {
      ...
      deps.logger.debug('cortex.command.upsertEntity', {
        id: entity.id,
        name: entity.name,
      });
    },
    ...
    deleteEntity: async (id: string) => {
      ...
      deps.logger.debug('cortex.command.deleteEntity', { id });
    },
    ...
    recordFact: async (input: RecordFactInput): Promise<string> => {
      ...
      deps.logger.debug('cortex.command.recordFact', {
        id: validated.id,
    ...
    linkNodes: async (
      ...
      deps.logger.debug('cortex.command.linkNodes', {
        id: edge.id,
        relation,
    ...
    markFactSuperseded: async (
      ...
      deps.logger.debug('cortex.command.markFactSuperseded', {
        oldFactId,
        newFactId,
      });
    },
    ...
    markMemorySuperseded: async (
      ...
      deps.logger.debug('cortex.command.markMemorySuperseded', {
        oldMemoryId,
        newMemoryId,
      });
    },
    ...
    storeContradiction: async (
      ...
      deps.logger.debug('cortex.command.storeContradiction', {
        id: validated.id,
