 ...
 * and storeFact from fact-store.ts. Cortex uses deps.structured.storeFact.
 * The extraction prompt is ported verbatim from KB fact-extractor.ts:20-31
 * (FACT_EXTRACTION_PROMPT, also used in Cortex's ingest.extractFacts).
 */

import { randomUUID } from 'node:crypto';
import type { Fact, FactCategory } from '@kybernesis/cortex-contracts';
